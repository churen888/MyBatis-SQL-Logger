...
### ✨ 新增功能
- **智能滚动模式 (Smart Scrolling)**: 全新设计的控制台滚动体验
  - 默认开启自动滚动，新日志到达时自动锁定到底部
  - 用户向上滚动查看历史时，自动暂停自动滚动，互不干扰
  - 用户滚回底部时，自动恢复自动滚动模式
  - 优化 `ScrollToBottomAction` 按钮状态，实时反映滚动模式
- **SQL 格式化增强**: 动态计算缩进宽度
  - `SELECT` / `SELECT DISTINCT` 等关键字后的字段对齐更加自然，根据关键字长度动态调整缩进

### 🐛 问题修复
- **修复 SQL 解析截断问题**:
  - 修复当字段名包含 `error` (如 `error_msg`) 时被误判为 ERROR 日志级别导致 SQL 截断的问题
  - 优化正则表达式，增加单词边界 `\b` 检测，避免关键词误匹配
- **修复自动滚动逻辑冲突**:
  - 引入 `isUpdating` 锁机制，解决日志追加时误判为用户手动滚动的问题
  - 优化滚动到底部的判定阈值（30px），提升滚动吸附的灵敏度
  - 修复点击"滚动到底部"按钮后自动滚动立即失效的问题
